...
DATA_PATH = os.environ.get('DATA_PATH', './data/Misar/MISAR_4D_Final.h5')
BASE_SAVE_DIR = "checkpoints/misar_4d_kfold"

# Experiment settings
NUM_FOLDS = 5          # 5-fold cross validation
BATCH_SIZE = 1024      # Recommended 1024/2048 for dual H100
...
# Model parameters
DIM_UNI = 1024
DIM_HIDDEN = 512       # Hidden Dim

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# ================= 1. Dataset with Section Filtering Support (MISAR) =================
class MISARDataset_Split(Dataset):
    def __init__(self, h5_path, allowed_sections, gene_indices=None):
        """
        allowed_sections: List of allowed sections (e.g., ['E11_0-S1', 'E13_5-S2'])
        gene_indices: Specified gene indices (ensure train/test gene consistency)
        """
        ...
        with h5py.File(h5_path, 'r') as f:
            # 1. If gene_indices not provided, calculate HVG (only on training set)
            ...
            # 2. Build indices (only include allowed_sections)
            for k in allowed_sections:
                if k not in f: continue
                # Must have uni_features
...
# ================= 2. Core Model Components (4D) =================

class FourierEncoding(nn.Module):
    # Input Dim = 4
    def __init__(self, input_dim=4, mapping_size=64, scale=10):
        super().__init__()
...
def run_fold(fold_idx, train_sections, test_sections):
    ...
    # 1. Build datasets
    train_dataset = MISARDataset_Split(DATA_PATH, allowed_sections=train_sections)
    test_dataset = MISARDataset_Split(DATA_PATH, allowed_sections=test_sections,
    ...
    # 2. Model
    model = MoEST_Plus_4D(num_genes=NUM_HVG, dim_hidden=DIM_HIDDEN)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in train_loader:
            ...
            loss = nb_criterion(y, mu, theta) + LAMBDA_COUPLE * coupled_gradient_loss(g, pos, grad)
...
if __name__ == "__main__":
    ...
    # 1. Get all section names
    print("Reading section list...")
    with h5py.File(DATA_PATH, 'r') as f:
        # Only take sections with uni_features
        all_keys = [k for k in f.keys() if k[0].isalpha() and 'uni_features' in f[k]]

    all_keys.sort()
    print(f"   Total Valid Sections: {len(all_keys)}")

    # 2. 5-fold cross validation (K-Fold Cross Validation)
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(kf.split(keys_array)):
        train_sec = keys_array[train_idx].tolist()
        test_sec = keys_array[test_idx].tolist()

        score = run_fold(fold_idx, train_sec, test_sec)
        results.append(score)

    print("\n" + "="*40)
    print("Final Experiment Results (5 Folds):")
    print(f"   Scores: {results}")
    print(f"   Average PCC: {np.mean(results):.4f} +/- {np.std(results):.4f}")
    print("="*40)
