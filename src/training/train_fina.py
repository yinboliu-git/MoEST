...
class MoEST_Misar(nn.Module):
    ...
    def forward(self, vis, pos, grad, lib, apply_msm=False):
        ...
        vis_rec = self.img_decoder(z)

        return mu, theta, self.func(z), self.align(z), vis_rec
    ...
    for fold, (tr, va) in enumerate(kf.split(sections)):
        ...
        for epoch in range(EPOCHS):
            ...
            for b in train_loader:
                ...
                original_vis = b["vis"].to(DEVICE) 
                
                mu, theta, g, z, vis_rec = model(
                    original_vis,
                    b["pos"].to(DEVICE),
                    b["grad"].to(DEVICE),
                    b["library_size"].to(DEVICE),
                ...
                loss_nb = LAMBDA_NB * nb_loss(b["y"].to(DEVICE), mu, theta)
                loss_align = LAMBDA_ALIGN * mse_loss(z, b["rna_z"].to(DEVICE))
                loss_grad = LAMBDA_GRAD * coupled_gradient_loss(g, b["pos"].to(DEVICE), b["grad"].to(DEVICE))
                loss_img = LAMBDA_IMG * mse_loss(vis_rec, original_vis) 
                
                loss = loss_nb + loss_align + loss_grad + loss_img
