...
BASE_SAVE_DIR = "checkpoints/her2_kfold_experiment"
...
class HER2Dataset_Split(Dataset):
    def __init__(self, h5_path, allowed_sections, gene_indices=None):
        ...
        # print(f"Initializing dataset with {len(allowed_sections)} sections...")
        ...
        with h5py.File(h5_path, 'r') as f:
            ...
            if gene_indices is None:
                ...
                # print(f"HVG calculation complete, locked {len(self.final_genes)} genes")
            else:
                self.gene_indices = gene_indices
                # print(f"Using inherited gene indices")
...
class MoEST_Plus(nn.Module):
    def __init__(self, num_genes, dim_uni=1024, dim_hidden=512): # Hidden changed to 512
        super().__init__()
...
#         all_keys = [k for k in f.keys() if k[0].isalpha() and 'gene' not in k and 'label' not in k]
    
#     # 移除可能存在的空切片（可选）
#     all_keys.sort()
#     print(f"   Total Sections: {len(all_keys)}")
...
#         train_sec = shuffled_keys[:split_idx]
#         test_sec = shuffled_keys[split_idx:]
        
#         # 运行这一折
#         score = run_fold(fold, train_sec, test_sec)
#         results.append(score)
    
#     print("\n" + "="*40)
#     print("📊 最终实验结果 (5 Folds):")
...
import argparse # 记得在文件最开头导入这个

# ... (前面的代码保持不变) ...

if __name__ == "__main__":
    # ================= Modified section: Parameter control =================
    parser = argparse.ArgumentParser(description='HER2 5-Fold Training')
    ...
    # Note: We must simulate 5-fold split, but only run the fold specified by args.fold
    seed = 42  # Global fixed seed to ensure consistent splits
    random.seed(seed)
    np.random.seed(seed)

    shuffled_keys = all_keys[:]
    random.shuffle(shuffled_keys) # Shuffle
